; Enhanced BASIC to assemble under 6502 simulator, $ver 2.22p5a
; Build with cc65 with:
;   cl65 -t none -l --feature labels_without_colons -o ehbasic.bin ehbasic.asm 2> ehbasic.log
...
; 2.22a	JGH: added conditional assembly for target, added BBC target, some bugfixes:
;  String variables can be assigned in direct mode
;  Text input allows CHR$8/CHR$127 and CHR$10/CHR$13
;  Svar correctly initialised to after end of program end marker
;  INPUT allows empty string on input, INPUT "prompt"; doesn't print "?"
;  FRE(0) returns unsigned integer
...
; 2.22p4a JGH: merged 2.22a and 2.22p4
...
; 2.22p5a JGH: merged 2.22p4a and 2.22p5
;              USR() defaults to calling (address), CALL/USR passes/returns registers
;		  MJM: NEO6502 additions
...
ZPROUT   =1			; Subroutines in zero page
WSSTART  =$0200		; Start of vector and input buffer workspace
;CODESTART=$2000		; Start of program code --NEO6502
IRQVEC   =$0
.endif
...
; zero page use
; -------------
LAB_WARM 	= ZPSTART+ZPWARM; $00=BASIC warm start entry point
Wrmjpl 		= LAB_WARM+1	; BASIC warm start vector jump low byte
Wrmjph 		= LAB_WARM+2	; BASIC warm start vector jump high byte

Usrjmp		= ZPSTART+$0A	; USR function JMP address
UsrrgA		= Usrjmp-3	; USR A register
UsrrgX		= Usrjmp-2	; USR X register
UsrrgY		= Usrjmp-1	; USR Y register
...
Rbyte4		= LAB_GBYT+22	; extra PRNG byte
.else
Bpntrl		= Sendh+1	; BASIC execute (get byte) pointer low byte
Bpntrh		= Bpntrl+1	; BASIC execute (get byte) pointer high byte

Rbyte4		= Sendh+1	; extra PRNG byte
.endif
...
.ifdef IRQVEC
NmiBase		= Rbyte4+4	; NMI handler enabled/setup/triggered flags
				; bit	function
				; ===	========
				; 7	interrupt enabled
				; 6	interrupt setup
...
Decss		= NmiBase+18	; number to decimal string start
.else
Decss		= Rbyte4+4	; number to decimal string start
.endif
Decssp1		= Decss+1	; number to decimal string start
ZPLastByte	= Decss+17

.if ZPLastByte>ZPEND
.error	"Too much zero page used"
.endif
...
TK_MOVE		= TK_CLS+1	; MOVE token
TK_DRAW		= TK_MOVE+1	; DRAW token
TK_PIXEL	= TK_DRAW+1	; PIXEL token
TK_COLOR	= TK_PIXEL+1	; COLOR token
...
.ifdef IRQVEC
Ibuffs		= IRQVEC+$14	; start of input buffer after IRQ/NMI code
...
LAB_COLD
; workspace initialisation, copy block to ccflag onwards
	LDX	#PG2_TABE-PG2_TABS-1	; byte count-1
LAB_2D13
	LDA	PG2_TABS,X	; get byte
	STA	ccflag,X	; store in workspace page
	DEX			; decrement count
	BPL	LAB_2D13	; loop if not done
	TXS			; reset stack pointer
	STX	Clineh		; set current line high byte (set immediate mode)

	JSR	MON_INIT	; target-specific initialisation
...
.else
	JSR	MON_LIMITS	; Ask MOS for memory limits
.endif
...
.endif
	...
	LDA	#<LAB_FREE	; point to sign-on message (low addr)
	LDY	#>LAB_FREE	; point to sign-on message (high addr)
	JSR	LAB_18C3	; print null terminated string from memory
	LDA	#<LAB_1274	; warm start vector low byte
	LDY	#>LAB_1274	; warm start vector high byte
	STA	Wrmjpl		; set warm start vector low byte
	STY	Wrmjph		; set warm start vector high byte
...
LAB_XERR
	...
	LDA	LAB_BAER+0,X	; get error message pointer low byte
...
; wait for Basic command (no "Ready" message)
LAB_127D
	JSR	LAB_1357	; call for BASIC input
LAB_1280
	STX	Bpntrl		; set BASIC execute pointer low byte
	STY	Bpntrh		; set BASIC execute pointer high byte
	JSR	LAB_GBYT	; get byte from Bpntr
	BEQ	LAB_127D	; loop while null

; go to interpret input line now ..
...
LAB_1319
;; to do: if first byte >=$20, text file loaded
;;      JSR   LAB_1477          ;; moved to LAB_133E: reset execution to start, clear vars and flush stack
...
LAB_133E
	TXA			; step past program end marker
	CLC
	ADC #2
	STA	Svarl		; set start of variables low byte
	LDA	ut1_ph		; get line start pointer high byte
	ADC	#0
	STA	Svarh		; set start of variables high byte
	JSR	LAB_1477	; reset execution to start, clear vars and flush stack
	JMP	LAB_127D	; just wait for Basic command, no "Ready"
...
; receive line from keyboard
				; $08 (BACKSPACE) and $7F (DELETE) as delete key
LAB_134B
	TXA
	BEQ	LAB_1359	; buffer is empty
	LDA	#$7F
	JSR	LAB_PRNA	; print the character
	DEX			; decrement the buffer counter (delete)
	.byte	$2C		; make LDX into BIT abs

; input a line of text (main entry point)
LAB_1357
...
LAB_1359
	...
	CMP	#BREAK		; compare with [CTRL-C/ESCAPE/BREAK]
	BNE	LAB_NOTBREAK
	JMP	LAB_BREAK
LAB_NOTBREAK
	CMP	#$07		; compare with [BELL]
	BEQ	LAB_137F	; branch if [BELL]
...
; next two lines ignore any nonprinting character and [SPACE] if input buffer empty
...
LAB_1374
	...
	BEQ	LAB_134B	; go to delete last character
	CMP	#$7F		; compare with [DELETE] (delete last character)
	BEQ	LAB_134B	; go to delete last character
...
LAB_137F
	JSR	LAB_PRNA	; print the character (returns NE if A<>0)
	BNE	LAB_1359	; loop for next character

LAB_1384
	JMP	LAB_1866	; do CR/LF, return to caller

; announce buffer full

LAB_138E
	LDA	#$07		; [BELL] character into A
	BNE	LAB_137F	; print the [BELL] but ignore input character
				; branch always
...
LAB_13A6
	LDY	#$FF		; set save index (makes for easy maths later)
	...
	TAX			; copy result to X (index past line number if any)
;;      STX   Oquote ; but X is no longer $00 ;; clear open quote/DATA flag
...
; *** end
	...
	CMP	#'<'		; compare with "<"
	BCS	LAB_13CC	; if >= is upper case or symbol, crunch

	CMP	#'0'		; compare with "0"
	BCS	LAB_13EC	; if >= is digit, save byte then continue crunching
	...
	BNE	LAB_13CC
	LDA	#$00
	BIT	Oquote		; test for "*" at start of statement
	BPL	LAB_1405	; start of statement, copy rest of line
	LDA	#'*'
...
LAB_CLEAR
	BEQ	LAB_147A	; if no following token go to "CLEAR"

				; else there was a following token (go to syntax error)
	RTS
...
; *** add
;.if (* & $FF) = $FD
	NOP			; push code on so return address of JSR on same page
;.endif  
...
; Program interupt check. This is called as a subroutine but exits back via a
...
LAB_1636
	CMP	#BREAK		; compare with CTRL-C/BREAK/ESCAPE/etc
...
LAB_CONT
	...
	BNE	LAB_166C	; go to continue if we can
...
.if TARGET=BBC
LAB_OSCLI
	LDX	Bpntrl		; get XY=>basic line
	LDY	Bpntrh
	INX
	BNE	LAB_OSCLI2
	INY
LAB_OSCLI2
	LDA	#$FF		; A=maximum string length
	JSR	BBCSTRG		; Convert <00>-string to <cr>-string
	JSR	$FFF7		; Call MOS command interpreter
.endif
...
.ifdef IRQVEC
	...
	JMP	LAB_SIRQ	; else go to set up IRQ
.endif
...
.ifdef IRQVEC
	...
	JMP	LAB_SNMI	; else go to set up NMI
.endif
...
; enter with A=first digit, carry clear
...
LAB_1785
	...
	INX			; else increment high byte
LAB_17B3
	JSR	LAB_IGBY	; increment and fetch from memory
...
LAB_17D5
	...
	CMP	#>Ibuffs
	BEQ	LAB_17FB	; string in immediate command
...
LAB_1831
	...
	JSR	LAB_296E	; convert FAC1 to string, returns YA=>string
...
LAB_CRLF
	...
	JSR	LAB_PRNA	; print the character
	LDA	#$0A		; load [LF]
	BNE	LAB_PRNA	; print the character and return, branch always
...
LAB_INPUT
	...
	JSR	LAB_1BC1	; evaluate string to Sutill/Sutilh
;;	JSR	LAB_18C6	; print string from Sutill/Sutilh
;;	JSR	LAB_GBYT
;;	CMP	#$3B
;;	BNE	LAB_193A	; Not ";", skip "?" prompt
;;	JSR	LAB_IGBY	; Step past ";"
	LDA	#$3B		; load A with ";"
	JSR	LAB_SCCA	; scan for CHR$(A), else do syntax error then warm start
	JSR	LAB_18C6	; print string from Sutill/Sutilh
	JMP	LAB_193A	; done with prompt, now get data
...
LAB_193A
	...
	JSR	LAB_1357	; get text input
	LDA	#$00		; set mode = INPUT
	BEQ	LAB_1953	; jump to read input data
;
; bugfix, allow empty string to be entered
...
LAB_1FB4
	JSR	LAB_GARB	; go to garbage collection
	SEC			; set carry for subtract
...
LAB_AYFC
	...
	SEC			; set exp=X, clear FAC1_3, normalise and return
	JMP	LAB_STFA	; ensure number returned as unsigned
...
LAB_CALL
	JSR	LAB_EVNM	; evaluate expression and check is numeric,
				; else do type mismatch
	JSR	LAB_USRVC
	JMP	LAB_GBYT	; get the next byte for the interpreter and return

; Default USR destination
LAB_USRVC
	LDA	#>(CallExit-1)	; set return address high byte
	PHA			; put on stack
	LDA	#<(CallExit-1)	; set return address low byte
	PHA			; put on stack
	JSR	LAB_F2FX	; convert floating-to-fixed
	LDA	UsrrgA		; fetch A from $0007
	LDX	UsrrgX		; fetch X from $0008
	LDY	UsrrgY		; fetch Y from $0009
	JMP	(Itempl)	; do indirect jump to user routine

; if the called routine exits correctly then it will return to here.

CallExit
	STA	UsrrgA		; fetch A from $0007
	STX	UsrrgX		; fetch X from $0008
	STY	UsrrgY		; fetch Y from $0009
	RTS
...
LAB_296E
	LDY	#$00		; set index = 1
	LDA	#$20		; prepare A=$20 for positive
	BIT	FAC1_s		; test FAC1 sign (b7)
	BPL	LAB_297B	; branch if +ve
	LDA	#$2D		; else character = "-"
	INY
	STA	Decss,Y		; store leading character "-" if negative
LAB_297B
	STA	FAC1_s		; clear FAC1 sign (b7)
...
LAB_2A91
	LDA	#<Decssp1	; get result string low pointer
	LDY	#>Decssp1	; get result string high pointer
	RTS
...
LAB_USR
	...
	JSR	LAB_PEEKY	; return byte in A
...
; automatically enable the interrupt when we exit
	...
	JMP	LAB_15C2	; go to interpreter inner loop
...
INGET
; shouldn't this check ccnull before V_INPT?
; shouldn't INPUT call this instead of directly calling V_INPT?
	JSR	V_INPT		; call scan input device
...
; these routines only enable the interrupts if the setup flag is set
...
LAB_SNMI
	...
	JMP	LAB_16F7	; else go to "Undefined statement" error and warm start
...
; Initialised on startup, can be overwritten by MON_INIT
PG2_TABS
	.byte	$00		; ctrl-c flag		-	$00 = enabled
	.byte	$00		; ctrl-c byte		-	GET needs this
	.byte	$00		; ctrl-c byte timeout	-	GET needs this
	.byte	$00		; spare byte, aligns to even
	.word	CTRLC		; ctrl c check vector	-	monitor may set this
;	.word	xxxx		; non halting key input	-	monitor must set this
;	.word	xxxx		; output vector		-	monitor must set this
;	.word	xxxx		; load vector		-	monitor must set this
;	.word	xxxx		; save vector		-	monitor must set this
;	.word	xxxx		; spare vector
PG2_TABE

; character get subroutine for zero page
; LAB_IGBY: step though memory to get next byte, skipping spaces
; LAB_GBYT: get current byte, skipping spaces
; callers expect X,Y to be preserved
; returns A=char, EQ=ELSE or : or [NUL], CC=digit, CS=not digit
; corrupts TempB
; This could be optimised by ensuring all calls allow Y to be corrupted or Y=0
; Looks like DIM and IF are the only callers that use Y
...
.if ZPROUT=0
LAB_IGBY			; don't put in zero page, don't self-modify
.endif
...
.else
	.byte	$00		; drop two bytes if 6510 target, I/O port at 0000/0001
.endif
	.byte	$4C		; JMP opcode
	.word	LAB_COLD	; initial warm start vector (cold start)
	.word	$0000		; 
	.word	$0000		; 
	.byte	$4C		; JMP opcode
	.word	LAB_USRVC	; initial user function vector, calls (FAC1)
...
LAB_SMSG
	.byte	"Enhanced BASIC ",VERSION,$0D,$0A,$00
LAB_FREE
...
LAB_CTBL
	...
	.word	LAB_MOVE-1	; MOVE		new command
	.word	LAB_DRAW-1	; DRAW		new command
	.word	LAB_PIXEL-1	; PIXEL		new command
	.word	LAB_COLOR-1	; COLOR		new command
	.word	LAB_PALETTE-1	; PALETTE	new command
	.word	LAB_VDU-1	; VDU new command
	.word   LAB_VREFRESH-1 ; REFRESH  new command
	.word	LAB_RECT-1
	.word   LAB_CIRCLE-1
	.word	LAB_TRIANGLE-1
	.word	LAB_PLAY-1
...
LBB_VSANE
	.byte	"ANE",TK_SANE		; SANE
LBB_SAVE
	.byte	"AVE",TK_SAVE		; SAVE
LBB_SGN
	.byte	"GN(",TK_SGN		; SGN(
LBB_SILENCE
	.byte	"ILENCE",TK_SILENCE	; SILENCE
LBB_SIN
	.byte	"IN(",TK_SIN		; SIN(
LBB_SPC
	.byte	"PC(",TK_SPC		; SPC(
...
LAB_KEYT
	...
	.word	LBB_VDRAW	; DRAW
	.byte	5,'P'
	.word	LBB_VPIXEL	; PIXEL
	.byte	5,'C'
	.word	LBB_VCOLOR	; COLOR
	.byte	7,'P'
	.word	LBB_VPALETTE ; PALETTE
	.byte	3,'V'
	.word	LBB_VVDU  	 ; VDU
	.byte	7,'R'
	...
	.word	LBB_VRECT	; RECT
	.byte	6,'C'
	.word	LBB_VCIRCLE	; CIRCLE
	.byte	8,'T'
	.word	LBB_VTRIANGLE	; TRIANGLE
	.byte	4,'P'
	.word	LBB_PLAY	; PLAY
	.byte	7,'S'
